...
#### Automated
- [x] 1.1 Migration file exists: `ls supabase/migrations/20260605000000_create_exercises_schema.sql`

#### Manual
- [x] 1.2 File is in correct directory and follows naming convention

### Phase 2: Define exercises table

#### Automated
- [ ] 2.1 Migration syntax is valid: `npx supabase db reset` succeeds
- [ ] 2.2 Table exists: `psql` query `\dt exercises` returns table
- [ ] 2.3 RLS enabled: `SELECT relrowsecurity FROM pg_class WHERE relname = 'exercises'` returns `t`
- [ ] 2.4 Check constraints work: attempt to insert invalid `exercise_type` fails
...
#### Automated
- [ ] 3.1 Migration runs successfully: `npx supabase db reset` completes without errors
- [ ] 3.2 Table exists: `psql` query `\dt exercise_completions` returns table
- [ ] 3.3 Foreign keys enforced: attempt to insert completion with invalid `exercise_id` fails
- [ ] 3.4 Index created: `\di idx_exercise_completions_user_date` returns index
- [ ] 3.5 RLS enabled: `SELECT relrowsecurity FROM pg_class WHERE relname = 'exercise_completions'` returns `t`
...
#### Automated
- [ ] 4.1 Migration runs successfully: `npx supabase db reset` completes without errors
- [ ] 4.2 Seed data exists: `SELECT COUNT(*) FROM exercises` returns `1`
- [ ] 4.3 Seed data is correct: `SELECT exercise_type, title FROM exercises WHERE id = 'a0000000-0000-0000-0000-000000000001'` returns `animated_pacer` and `Database Performance Fundamentals`

#### Manual
- [ ] 4.4 Content length is appropriate: ~450 words
- [ ] 4.5 Content is technical and developer-relevant
- [ ] 4.6 JSONB config is valid: `SELECT config->>'target_wpm'` returns `250`

### Phase 5: Verify migration

#### Automated
- [ ] 5.1 Database reset succeeds: `npx supabase db reset` exits 0
- [ ] 5.2 No schema drift: `npx supabase db diff` returns no differences
- [ ] 5.3 Tables created: `npx supabase db dump --data-only --schema public -t exercises -t exercise_completions` succeeds
- [ ] 5.4 RLS enabled on both tables: SQL query confirms `relrowsecurity = true` for both
...
- [ ] 5.7 Foreign key constraints work: Attempt to insert completion with non-existent exercise_id fails
