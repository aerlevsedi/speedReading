...
# Shape Notes: Speed-Reading Training App
...
**Person:** Developers reading code (primary persona), with potential expansion to students and general heavy readers.

**Moment:** When they need to find information quickly in large volumes of text (books, documentation, code files) and understand main ideas without reading every word.

**Cost today:** Time, energy, and money wasted on slow reading. Lack of structured practice. No proof of progress. Unclear if/when they've mastered speed-reading and can stop using the app.

**Pain category:** Missing capability — the skill is absent, and current tools don't provide a structured learning path with measurable progress and a graduation point.

**Insight:** Existing speed-reading apps lack (1) progress proof / graduation signal ("you learned it, now stop"), and (2) domain-specific exercises (code snippets, technical documentation for developers). Building for multiple user types is acknowledged as a hard task — domain-specificity may need to be scoped carefully in MVP.

**Key qualities surfaced:**
- Pleasant + gamification (users are tired after work/studying)
- Visible progress with a graduation point (proof they learned something)
...
1. **Performance:** Dashboard and exercise views load quickly — perceived as fast by users on typical broadband connections (target: < 2 seconds for initial load).
...
4. **Privacy:** Only authenticated users can access the dashboard and personal exercise history. No unauthenticated access to user data (already captured in Access Control guardrail).
...
4. **Native mobile app** — no iOS/Android native app, no hybrid app framework (React Native, Flutter, etc.). Web app accessed via desktop browsers is primary; responsive design is acceptable if it doesn't add complexity, but native mobile app development is out of scope.
